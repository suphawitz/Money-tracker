...
// --- State ---
const currentPage = ref('home'); // home, categories, dashboard
...
// 2. Logic สำหรับหน้า Dashboard (Filter & Charts)
...
// 2.1 Mission 2026 Logic
const mission2026Data = computed(() => {
  const missionList = transactions.value.filter(t => 
    t.type === 'เงินออม' && t.category === 'Mission 2026'
  );
  // กรองตามวันที่ที่เลือกใน Dashboard ด้วย
...
// 2.2 Chart Data Generator Helper
const getChartDataByType = (type) => {
  ...
  return {
    ...
    datasets: [{
      backgroundColor: ['#41B883', '#E46651', '#00D8FF', '#DD1B16', '#FFC0CB', '#FFD700', '#8A2BE2'],
...
const barData = computed(() => ({
  ...
  datasets: [{
    ...
    data: [dashSummary.value.income, dashSummary.value.expense, dashSummary.value.savings]
  }]
}));
    ...
    <div class="max-w-2xl mx-auto p-4">
      ...
      <div v-if="currentPage === 'home'" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
           ...
           <form @submit.prevent="submitTransaction" class="space-y-3">
              ...
              <input type="text" v-model="form.name" placeholder="ชื่อรายการ (Option)" class="w-full text-right text-sm text-gray-500 outline-none">
        ...
        <div class="space-y-3">
           ...
           <div v-for="(item, i) in homeTransactions" :key="i" class="bg-white p-3 rounded-xl flex justify-between items-center border border-gray-100 shadow-sm">
              <div class="flex items-center gap-3">
                 ...
                 <div><p class="font-bold text-gray-800 text-sm">{{ item.category }}</p><p class="text-xs text-gray-400">{{ formatDate(item.date) }} <span v-if="item.name">- {{ item.name }}</span></p></div>
              </div>
      ...
      <div v-if="currentPage === 'dashboard'" class="space-y-6 animate-fade-in">
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
           ...
           <div class="flex gap-2">
              <select v-model="summaryMonth" class="bg-gray-100 rounded-lg px-3 py-2 text-sm font-bold outline-none">
                 <option value="all">ทุกเดือน</option>
                 <option v-for="(m, i) in months" :key="i" :value="i">{{ m }}</option>
              </select>
              <select v-model="summaryYear" class="bg-gray-100 rounded-lg px-3 py-2 text-sm font-bold outline-none">
        ...
        <div class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
           <div class="relative z-10">
              <div class="flex justify-between items-start">
                 <div>
                    <h3 class="font-black text-2xl italic tracking-wider">🚀 MISSION 2026</h3>
                    <p class="text-white/80 text-xs">เป้าหมายเงินออมแห่งอนาคต</p>
                 </div>
                 <div class="text-right">
                    <p class="text-xs text-white/80">ยอดสะสมรวม</p>
                    <p class="text-3xl font-black">{{ formatMoney(missionTotal) }} <span class="text-sm font-normal">บาท</span></p>
                 </div>
              </div>
              ...
              <div class="mt-6 bg-white/10 rounded-xl p-4 backdrop-blur-sm max-h-40 overflow-y-auto">
                 ...
                 <div v-else class="space-y-2">
                    <div v-for="(t, i) in mission2026Data" :key="i" class="flex justify-between text-sm border-b border-white/10 pb-1 last:border-0">
        ...
        <div class="grid md:grid-cols-2 gap-4">
           <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <h3 class="font-bold text-gray-700 mb-2 text-center text-sm">สัดส่วนรายจ่าย</h3>
              <div class="h-48"><Pie :data="getChartDataByType('รายจ่าย')" :options="chartOptions" /></div>
           </div>
           <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
              <h3 class="font-bold text-gray-700 mb-2 text-center text-sm">ที่มาของรายได้</h3>
              <div class="h-48"><Pie :data="getChartDataByType('รายรับ')" :options="chartOptions" /></div>
           </div>
           <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 md:col-span-2">
              <h3 class="font-bold text-gray-700 mb-2 text-center text-sm">พอร์ตเงินออม</h3>
              <div class="h-48"><Pie :data="getChartDataByType('เงินออม')" :options="chartOptions" /></div>
           </div>
        </div>
      </div>
      ...
      <div v-if="currentPage === 'categories'" class="bg-white rounded-2xl shadow-sm p-6 space-y-6">
         ...
         <div class="bg-gray-50 p-4 rounded-xl border">
            ...
            <form @submit.prevent="submitCategory" class="space-y-3">
               <div class="grid grid-cols-3 gap-2">
                  <button type="button" v-for="t in ['รายจ่าย','รายรับ','เงินออม']" :key="t" @click="catForm.type=t" :class="catForm.type===t?'bg-black text-white':'bg-white border text-gray-400'" class="py-2 rounded-lg text-xs font-bold transition-all">{{ t }}</button>
               </div>
               <input type="text" v-model="catForm.name" placeholder="ชื่อหมวดหมู่ (เช่น Mission 2026)" required class="w-full p-2 border rounded-lg text-sm">
