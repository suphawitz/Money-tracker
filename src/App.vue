...
// ⚠️ ใส่ URL ล่าสุดของคุณที่นี่ (อย่าลืมเปลี่ยนนะครับ!)
...
// ตั้งค่า Default เป็นเดือนและปีปัจจุบัน
const currentDate = new Date();
const selectedMonth = ref(currentDate.getMonth()); // 0 = ม.ค., 11 = ธ.ค.
...
// --- Computed: หัวใจสำคัญของระบบ Filter ---

// 1. กรองข้อมูลตาม เดือน/ปี ที่เลือก
...
// 2. คำนวณยอด จากข้อมูลที่กรองแล้วเท่านั้น
  ...
  <div class="min-h-screen bg-gray-100 p-4 md:p-8 font-sans">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <h1 class="text-2xl font-bold text-gray-800">💰 Money Tracker</h1>
        
        <div class="flex gap-2 bg-white p-2 rounded-lg shadow-sm">
          <select v-model="selectedMonth" class="bg-transparent font-bold text-gray-700 outline-none cursor-pointer">
            <option v-for="(m, index) in months" :key="index" :value="index">{{ m }}</option>
          </select>
          <select v-model="selectedYear" class="bg-transparent font-bold text-gray-700 outline-none cursor-pointer">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6 order-2 md:order-1">
          <div class="grid grid-cols-2 gap-4">
            <div class="bg-green-100 p-4 rounded-2xl">
              <p class="text-xs text-green-600 font-bold uppercase">รายรับ ({{ months[selectedMonth] }})</p>
              <p class="text-2xl font-bold text-green-800">+{{ totalIncome.toLocaleString() }}</p>
            </div>
            <div class="bg-red-100 p-4 rounded-2xl">
              <p class="text-xs text-red-600 font-bold uppercase">รายจ่าย ({{ months[selectedMonth] }})</p>
            ...
            <div class="bg-white border border-gray-200 p-4 rounded-2xl">
              <p class="text-xs text-gray-500 font-bold uppercase">คงเหลือ ({{ months[selectedMonth] }})</p>
          ...
          <div class="bg-white rounded-2xl shadow-sm p-6">
            <h3 class="font-bold text-gray-800 mb-4">รายการในเดือน {{ months[selectedMonth] }} {{ selectedYear }}</h3>
            
            <div v-if="filteredTransactions.length === 0" class="text-center text-gray-400 py-10">
              ไม่มีรายการในเดือนนี้
            </div>

            <div v-else class="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              <div v-for="(item, index) in filteredTransactions" :key="index" class="flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg border-b border-gray-100 last:border-0">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 h-fit order-1 md:order-2">
          <h2 class="text-xl font-bold text-gray-800 mb-4">📝 บันทึกใหม่</h2>
          <form @submit.prevent="submitForm" class="space-y-4">
            ...
            <div>
              ...
              <div class="flex gap-2">
                <input type="text" v-model="form.category" placeholder="หมวดหมู่ (เช่น อาหาร)" required class="w-1/2 p-2 border rounded-lg text-sm">
                <input type="text" v-model="form.name" placeholder="ชื่อรายการ" class="w-1/2 p-2 border rounded-lg text-sm">
              </div>
            </div>
